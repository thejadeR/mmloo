$(function () {
    ...
    // 加操作
    $('.addminus .add').click(function () {
        var num1 = $('.addminus .num').html()
        // console.log(num1)
        num1 = parseInt(num1) + 1
        if (num1) {  // 显示
            $('.addminus .mtp').show()
            $('.addminus .num').show()
            $('.addminus .num').html(num1)
        } else {    // 隐藏
            $('.addminus .mtp').hide()
            $('.addminus .num').hide()
        }
    ...
    // 减操作
    $('.addminus .mtp').click(function () {
        var num2 = $('.addminus .num').html()
        // console.log(num2)
...
// 加到购物车操作
    $('.f-add-2 .addgds').click(function () {
        ...
        $.get('/addcart/', {'goodsid': goodsid, 'numbers': $('.addminus .num').html()}, function (response) {
            ...
            if (response.status == -1) {     // 未登录，直接跳转到登录
                ...
                // alert('添加成功')
                $('.f-add-2 .msg').html('添加成功，快去购物车查看吧！')
                $('.f-add-2 .msg').fadeOut(500)
                location.reload()
